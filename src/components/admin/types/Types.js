...
export default function Types() {
    ...
    const deleteItem = (e, item) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`/delete-type/${item._id}`).then(res => {
    ...
    if (itemsList.length > 0) {
        items_HTMLTABLE = <>
            <div className='mx-0 grid grid-cols-12 text-center break-all'>
                <div className='pb-3 hidden md:block text-start col-span-9'>Name</div>
                ...
                {itemsList.map((item) => {
                    return (
                        ...
                        <div key={item._id} className="mx-0 grid grid-cols-12 text-center break-all">
                            <div className='pt-3 hidden md:block text-start col-span-9'>{item.name}</div>
        ...
        items_HTMLTABLE =
            <div className="flex flex-col gap-4 items-center justify-center text-center h-[25vh]">
                ...
                <h3 className='text-2xl font-bold'>Theres no Types</h3>
    ...
    return (
        <>
            <div className="max-w-[80rem] p-5 mx-auto">
                <div className='rounded-lg shadow-lg'>
                    <div className="flex justify-between items-center bg-gray-100 p-3 rounded-t-lg">
                        <h5 className='mb-3 mb-0'>Types ( {itemsList.length} )</h5>
                        <button type='button' className='bg-blue-600 text-white p-2 rounded' onClick={() => { setModalShow(true) }}>Add Type</button>
                    </div>
                    <div className="p-5">
            ...
            <AddTypeModal show={modalShow} hide={() => { setModalShow(false); getItems() }} />
            <EditTypeModal show={editModalShow} hide={() => { setEditModalShow(false); getItems() }} toedit={toEdit} />
        </>
    )
