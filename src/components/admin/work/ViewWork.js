...
export default function ViewWork() {
    ...
    const deleteItem = (e, item) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`/delete-work/${item._id}`).then(res => {
    ...
    if (itemsList.length > 0) {
        items_HTMLTABLE = <>
            <div className='mx-0 grid grid-cols-12 text-center break-all gap-2'>
                <div className='pb-3 hidden md:block text-start col-span-2'>Name</div>
                <div className='pb-3 col-span-3 md:col-span-2 text-start'>Description</div>
                <div className='pb-3 col-span-3 md:col-span-2 text-start'>Techs</div>
                <div className='pb-3 col-span-3 md:col-span-2 text-start'>Image</div>
                <div className='pb-3 col-span-3 md:col-span-2 text-start'>Link</div>
            ...
            <div className='divide-y'>
                {itemsList.map((item) => {
                    return (
                        ...
                        <div key={item._id} className="mx-0 grid grid-cols-12 text-center break-all gap-2">
                            <div className='pt-3 hidden md:block text-start col-span-2'>{item.name}</div>
                            <div className='pt-3 col-span-3 md:col-span-2 text-start'>{item.description}</div>
                            <div className='pt-3 col-span-3 md:col-span-2 text-start'>{item.technologies}</div>
                            <div className='pt-3 col-span-3 md:col-span-2 text-start'><img src={`${process.env.REACT_APP_URL}/${item.image}`} /></div>
                            <div className='pt-3 col-span-3 md:col-span-2 text-start'>{item.link}</div>
        ...
        items_HTMLTABLE =
            <div className="flex flex-col gap-4 items-center justify-center text-center h-[25vh]">
                ...
                <h3 className='text-2xl font-bold'>Theres no Work</h3>
    ...
    return (
        <>
            <div className="max-w-[80rem] p-5 mx-auto">
                <div className='rounded-lg shadow-lg'>
                    <div className="flex justify-between items-center bg-gray-100 p-3 rounded-t-lg">
                        <h5 className='mb-3 mb-0'>Work ( {itemsList.length} )</h5>
                        <button type='button' className='bg-blue-600 text-white p-2 rounded' onClick={() => { setModalShow(true) }}>Add Work</button>
                    </div>
                    <div className="p-5">
            ...
            <AddWorkModal show={modalShow} hide={() => { setModalShow(false); getItems() }} />
            <EditWorkModal show={editModalShow} hide={() => { setEditModalShow(false); getItems() }} toedit={toEdit} />
        </>
    )
